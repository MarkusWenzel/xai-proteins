...
import transformers
from prot_go_mod import ProteinClassifier
...
def run_embedding_all_proteins(data_path:str):
    """
    Store embedding attribution for every sample in embedding_attribution.csv
    Called by: ig_embedding.py
    Argument: data_path: either "data/clas_go_deepgoplus_temporalsplit/test.json" or "data/clas_go_deepgoplus_cafa/test.json"
    """
    model, output_dir, prot_df = ig_setup(data_path)
...
def run_attention_attribution_all_proteins(n_steps:int, hooks:list, data_path:str):
    """
    Run integrated gradient for each sample and retrieve the attribution at every attention mechanism.
    Called by: ig.py    
    Args:
        ...
        data_path: either "data/clas_go_deepgoplus_temporalsplit/test.json" or "data/clas_go_deepgoplus_cafa/test.json"
    """
    model, output_dir, prot_df = ig_setup(data_path)
...
def ig_setup(data_path: str = "data/clas_go_deepgoplus_temporalsplit/test.json"):
    """ 
    Integrated Gradient setup of model, tokenizer and data. 
    Get model and protein dataframe for the datapath and output path. Return: model, output_dir, prot_df 
    """
    ...
    prot_df=pd.DataFrame(data[0], columns=["sequence","name","label"])
    ...
    if data_path == "data/clas_go_deepgoplus_temporalsplit/test.json":
        go_path = Path("data-2016") # clas_go_deepgoplus_temporalsplit
    elif data_path == "data/clas_go_deepgoplus_cafa/test.json":
        go_path = Path("data-cafa") # clas_go_deepgoplus_cafa   

    # Select label dimensions in multilabel GO label vector: (GO-term)=1 vs. rest =0
    label_itos = np.load(Path(data_path).parent/"label_itos.npy")
    selected_label_dim = np.where(label_itos == SELECTED_GO_TERM)[0][0]
    prot_df['label'] = prot_df['label'].apply(lambda x: bool(int(x[selected_label_dim])))

    # Keep only proteins that are annotated with the selected GO-term (positive class)
    prot_df = prot_df.loc[ prot_df['label'] == True]
    prot_df['label'] = selected_label_dim    

    split = os.path.split(os.path.dirname(data_path))[1] # 'clas_go_deepgoplus_cafa' or 'clas_go_deepgoplus_temporalsplit'
    model = get_model(split=split)
...
def get_model(split):
    """
    Gets the fine-tuned model from the directory specified by split
    Parameter: split: str = "clas_go_deepgoplus_cafa" or "clas_go_deepgoplus_temporalsplit"
    Return: model cuda zero_grad eval
    """

    current_dir = os.path.dirname(__file__)
    path = Path(current_dir).resolve()
    pretrained_model_path = f"{path}/models/{split}/checkpoints/"
...
def get_model_inputs(prot_seq, prot_label, model):
    ...
    Returns: Tensors input_ids, ref_input_ids, target
    """
...
def attribution_to_csv(attribution: Tensor, output_dir: str, protein_name: str, part:str) -> None:
    """
    Writing csv from 30xNx1024 total_attribution tensor.
    Split attribution of size 30xNx1024 in individual heads and write in csv 480 rows of length N.
...
def ig_embedding(model, input_ids, ref_input_ids, target, n_steps: int = 50, attribute_to_layer_input: bool = False, module_name: str = "embeddings", method = "gausslegendre", hooks:list = [1], **kwargs):
    ...
    def forward_func(inputs, token_type_ids=None, attention_mask=None):
        pred_logit = model(inputs, token_type_ids=token_type_ids, attention_mask=attention_mask)
...
def run_ig(forward_func, module, input_ids, ref_input_ids, target, n_steps, method, return_convergence_delta, attribute_to_layer_input):
    """ Runs integrated gradient; returns: attribution or attribution,delta """
