...
which was published under the Apache-2.0 license, https://github.com/UKPLab/sentence-transformers/blob/master/LICENSE
We cite both aforementioned licenses below the code.
"""
     
class DefaultPool(nn.Module):
    def __init__(self,embeddings_dim,pool_cls=True, pool_max=False, pool_mean=False, pool_mean_sqrt=False):
        super().__init__()
        ...
        n_dim = int(pool_cls)+int(pool_max)+int(pool_mean)+int(pool_mean_sqrt)
        assert(n_dim>0)
        self.output_dim = n_dim*embeddings_dim

    def forward(self,token_embeddings, attention_mask,**kwargs):
    	#token_embeddings has shape bs,seq,feat
        output_vectors = []
        if self.pool_cls:
        ...
        if self.pool_mean or self.pool_mean_sqrt:
            input_mask_expanded = attention_mask.unsqueeze(-1).expand(token_embeddings.size()).float()
            sum_embeddings = torch.sum(token_embeddings * input_mask_expanded, 1)

            #If tokens are weighted (by WordWeights layer), feature 'token_weights_sum' will be present
            if "token_weights_sum" in kwargs.keys():
                sum_mask = kwargs["token_weight_sum"].unsqueeze(-1).expand(sum_embeddings.size())
            else:
...
def create_head(nf, nc, lin_ftrs=None, dropout=0.1, norm=True, act="relu", layer_norm=True, final_act=""):
    "Model head that takes `nf` features, runs through `lin_ftrs`, and about `nc` classes"
    lin_ftrs = [nf,nc] if lin_ftrs is None else [nf] + lin_ftrs + [nc]
    ps = [dropout] if not isinstance(dropout,Iterable) else dropout
    if len(ps)==1: ps = [ps[0]/2] * (len(lin_ftrs)-2) + ps
    
    actns = [nn.ReLU(inplace=True) if act=="relu" else nn.GELU()] * (len(lin_ftrs)-2) + [None]
    layers = [] # New
    for ni,no,p,actn in zip(lin_ftrs[:-1],lin_ftrs[1:],ps,actns):
        layers += bn_drop_lin(ni,no,norm,p,actn,layer_norm=layer_norm)
...
def bn_drop_lin(n_in, n_out, bn=True, p=0., actn=None, layer_norm=False):
    "Sequence of batchnorm/layernorm (if `bn`), dropout (with `p`) and linear (`n_in`,`n_out`) layers followed by `actn`."
